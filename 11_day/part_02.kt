...
fun synchronizedFlashOccurred(grid: OctopusGrid): Boolean {
    return grid.all { row -> row.all { it == 0 } }
}

fun findSynchronizedFlashStep(grid: OctopusGrid): Int {
    var currentGrid = grid
    var step = 0

    while(!synchronizedFlashOccurred(currentGrid)) {
        currentGrid = singleStep(currentGrid).grid
        step += 1
    }
...
println(findSynchronizedFlashStep(initialGrid))